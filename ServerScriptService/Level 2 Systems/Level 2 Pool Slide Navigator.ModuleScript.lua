-- PRIVATE GIANT NAVIGATOR: derived from v1767; not installed or approved for publish.
...
-- Giant-only derivative: the production Pool Foam module is unchanged.
...
function Navigator.PrepareContext(manifest, cancelled)
 assert(manifest and manifest.World and manifest.Layout, "manifest required")
 local world = manifest.World
 local generation = world:GetAttribute("Level2_Generation")
 local context = {World=world, Generation=generation, GroundParts={}, BuoyantProps={},
  Visited=0, Slices=0, MaxSlice=0, Prepared=false}
 local began, sliceAt, sliceCount = os.clock(), os.clock(), 0
 local stack = {world}
 while #stack > 0 do
  if (cancelled and cancelled()) or not world.Parent
   or world:GetAttribute("Level2_Generation") ~= generation then return nil, "context preparation cancelled" end
  local current = table.remove(stack)
  context.Visited += 1; sliceCount += 1
  if current:IsA("BasePart") and current.CanCollide
   and current:GetAttribute("Level2_EntityGround") == true then table.insert(context.GroundParts,current) end
  if current:GetAttribute("Level2_BuoyantProp") == true then table.insert(context.BuoyantProps,current) end
  if current.Name == "Level 2 Navigation" then context.NavigationFolder=current end
  for _,child in ipairs(current:GetChildren()) do table.insert(stack,child) end
  local elapsed=os.clock()-sliceAt
  -- Amortize preparation across bounded 6 ms slices: tiny 2 ms/256-node
  -- batches spent almost all startup time waiting for heavily loaded MP frames.
  if sliceCount >= 1024 or elapsed >= .006 then
   context.MaxSlice=math.max(context.MaxSlice,elapsed);context.Slices+=1
   task.wait();sliceAt=os.clock();sliceCount=0
  end
 end
 context.MaxSlice=math.max(context.MaxSlice,os.clock()-sliceAt)
 context.Elapsed=os.clock()-began
...
function Navigator.new(model, manifest, tuning, options)
 ...
 local queryContext=options.PreparedContext
 assert(type(queryContext)=="table" and queryContext.Prepared==true
  and queryContext.World==manifest.World
  and queryContext.Generation==manifest.World:GetAttribute("Level2_Generation"),
  "Pool Slide Navigator requires current generation's prepared query context")
	...
	local navigationFolder = queryContext.NavigationFolder
	...
	for _, descendant in ipairs(queryContext.BuoyantProps) do addExclusion(descendant) end
	...
	local groundParts = queryContext.GroundParts
	...
	self.Tuning.GoalArrivalDistance = readNumber(tuning, "GoalArrivalDistance",
		self.Tuning.WaypointArrivalDistance, .05, self.Tuning.WaypointArrivalDistance)
	return self
end
...
function Navigator:_surfaceAt(position, allowLargeStep, expectedFootY)
	if not planningCheckpoint() then return nil end
...
function Navigator:_bodyBoxClear(targetFoot): boolean
	if not planningCheckpoint() then return false end
...
function Navigator:_reachedGoal()
	...
	return horizontalDistance(self.FootPosition, target) <= (self.Tuning.GoalArrivalDistance or self.Tuning.WaypointArrivalDistance)
...
function Navigator:_shortAtomicJoin(points, requestStart)
	...
	-- Giant-only bounded atomic join: at most one nearby endpoint and one
	-- short projected connector. Long smoothing was certified in yielding work.
	local candidate=points[nearestIndex]
	if candidate and horizontalDistance(from,candidate)<=6
		and self:_walkingEdgeClear(from,candidate) then
		local joined={}
	...
	if endpoint then
		...
		local ahead = span > .001 and nearestPoint:Lerp(endpoint, math.min(1,6/span)) or endpoint
		if horizontalDistance(from,ahead) <= 8 and self:_walkingEdgeClear(from,ahead) then
			local joined = {ahead}
			for rest=nearestIndex,#points do table.insert(joined,points[rest]) end
			return joined, nearestIndex-1
		end
...
function Navigator:_certifiedJoin(points, requestStart)
	local from = self.FootPosition
	local segmentStart, nearestPoint = requestStart, requestStart
	local nearestIndex, minimum = 1, math.huge
	-- Find current progress on the polyline even after a long async delay.
...
-- Certify furthest shortcut cooperatively; atomically bridge latest foot over at most eight studs.
...
function Navigator:_rejectStableRoute(reason)
 ...
 if context and os.clock()>=context.Deadline then
  ...
  return -- preserve certified incumbent; Step validates every stride
	...
	local keepIncumbent = nextPoint and self:_walkingEdgeClear(self.FootPosition, nextPoint)
...
function Navigator:_requestPath(goal, graphOnly)
	...
	task.defer(function()
		...
		-- Goal/approach standability above can now yield. Validate ownership
		-- again before any shared route state is mutated below.
		if self.Destroyed or requestId ~= self.RequestId then return end
		...
		if stable then
			...
			if valid then
				...
				local currentGoal=self.Goal
				local planned=Vector3.new(goal.X-self.FootPosition.X,0,goal.Z-self.FootPosition.Z)
				local current=currentGoal and Vector3.new(currentGoal.X-self.FootPosition.X,0,currentGoal.Z-self.FootPosition.Z)
				if current and horizontalDistance(goal,currentGoal)>=self.Tuning.RepathDistance
					and (planned:Dot(current)<0 or horizontalDistance(goal,currentGoal)>math.max(24,current.Magnitude*.5)) then
					self.Waypoints={};self.GoalApproach=nil;self.Computing=false
					self.RouteRejectedCount+=1;self.LastFailure="target changed direction during sliced join"
					return
				end
			end
			if not joined then
...
function Navigator:Step(deltaTime, speed)
	...
	while waypoint do
		...
		local arrivalDistance = self.WaypointIndex == #self.Waypoints
			and (self.Tuning.GoalArrivalDistance or self.Tuning.WaypointArrivalDistance)
			or self.Tuning.WaypointArrivalDistance
		if difference.Magnitude > arrivalDistance then break end
		self.WaypointIndex += 1
	...
	if not waypoint then
		...
		local reached = self.Tuning.StableRoutes and self:_reachedGoal()
			or (not self.Tuning.StableRoutes and self.Goal ~= nil
				and horizontalDistance(self.FootPosition, arrivalTarget) <= (self.Tuning.GoalArrivalDistance or self.Tuning.WaypointArrivalDistance))
