...
if devAllowed and pages.Dev then
	...
	local controls = {
		...
		{
			Name = "PULL TWO PUMPS",
			Description = "Pulls the next lever now, then the second 5 seconds later.",
			Command = "level2PumpPair",
			Action = true,
			Level = 2,
			ActionCaption = "PULL",
	...
	for order, info in ipairs(controls) do
		...
		local function actionAvailable()
			...
			return workspace:GetAttribute("SelectedLevel") == (info.Level or 3)
				...
				and player:GetAttribute("InRound") == true
				and (not info.BusyAttribute or player:GetAttribute(info.BusyAttribute) ~= true)
		end
		local function refresh()
			if info.Action then
				local available = actionAvailable()
				local busy = info.BusyAttribute and player:GetAttribute(info.BusyAttribute) == true
	...
	player:GetAttributeChangedSignal("DevLevel2PumpSerial"):Connect(function()
		...
		local message = messages[status]
		if not message and status:find("CANCELLED_", 1, true) == 1 then
			message = "Pump sequence cancelled because the round or character changed."
		end
		local success = status == "FIRST_PULLED_SECOND_IN_5_SECONDS" or status == "TWO_PUMPS_PULLED"
		showStatus(message or ("Pump sequence: " .. status), success and "success" or "error")
	end)
...
-- A mounted page is owned by its own ModuleScript (RECORDS today; Daily Rewards
