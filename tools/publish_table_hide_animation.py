...
local status,id=AssetService:CreateAssetAsync(sequence,Enum.AssetType.Animation,{
 Name="Zyntra Table Hide Hold v1",
...
def main():
    ...
    try:
        ...
        result=client.call('execute_luau',{'datamodel_type':'Edit','code':source(payload)})
        (OUT/'publish-attempt-result.txt').write_text(str(result))
