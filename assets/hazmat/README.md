...
On 24 September, Codex uploaded the six color maps and wheel art as **Images owned by ERN Roblox Studios** and imported six textured variants of the canonical rig into `ServerStorage`. The wheel, shop, ownership, and cosmetic visual code match Studio and the repository; **none of this hazmat work has been published**. The current visual uses the game's R15 motion as input for client-side Bone retargeting. Native Play, a two-client round, and phone/tablet simulation passed the visual checks described in [CLAUDE_INTEGRATION_HANDOFF.md](CLAUDE_INTEGRATION_HANDOFF.md). Paid-purchase, DataStore rejoin, and physical-device tests remain. The older instruction in `docs/CLAUDE_ALL_ACTIVE_TRELLO_2026-09-23.md` to skip skins was superseded by the owner's later request.
