...
On 24 September, Codex uploaded the six color maps and wheel art as **Images owned by ERN Roblox Studios** and imported six textured variants of the canonical rig into `ServerStorage`. The wheel, shop, ownership, and cosmetic visual code is in Studio and the repository under test; **none of this hazmat work has been published**. The current visual uses the game's R15 motion as input for client-side Bone retargeting. Its movement and cosmetic attachments still need final visual acceptance, a real-round/multiplayer check, and device checks. The shop now has a polished, auto-rotating 3D viewport of the selected suit; native Play QA of that viewport is still in progress. See [CLAUDE_INTEGRATION_HANDOFF.md](CLAUDE_INTEGRATION_HANDOFF.md) for exact asset IDs and verification status. The older instruction in `docs/CLAUDE_ALL_ACTIVE_TRELLO_2026-09-23.md` to skip skins was superseded by the owner's later request.
...
The two premium toppers are separate static GLBs: Static Wraith is 2,897 triangles and False Sun is 3,048. Both have been imported into Roblox and are attached as cosmetics by `HazmatSkinVisuals`; final placement and visual acceptance are still pending. Keep hitboxes, detection, movement speed, noise, interaction reach, camera obstruction, and gameplay light radius unchanged. The toppers do not include actual animated effects.
...
Offline checks passed: 23 skin checks, 401 daily-rewards checks, and 613 Lucky Wheel client checks. A solo Studio Play session showed the shop's SKINS cards, a Pool Service Token purchase and equip, a six-sector wheel, and a normal potion spin/claim without console errors. This does not verify the new 5% outcome in a live spin, paid purchase, DataStore rejoin, multiplayer replication, or physical phone/tablet behavior. The imported suit's native-R15-to-Meshy deformation, topper placement, and auto-rotating `ViewportFrame` are under native Play QA. The work is **not published**, and multiplayer, persistence, commerce, device, and real-round checks remain open; the Trello card must stay open until the release gates pass.

Before another Studio edit, follow `AGENTS.md`: read fresh Source and editor-source, preserve other developers' work, and mirror only verified changes. The native pre-import backup is `artifacts/hazmat-20260924/studio-before-hazmat.rbxl`. The current repository manifest is an inventory of canonical files, not proof of whole-place parity; the eight newly mirrored Level 5 scripts alone do not establish whole-place sync.
