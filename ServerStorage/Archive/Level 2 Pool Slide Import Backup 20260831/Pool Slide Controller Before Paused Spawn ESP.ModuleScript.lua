-- Third-pump encounter. This owns only the large Pool Slide entity; the
-- second-pump Slidemouth and its existing escalation remain independent.
...
local function setTarget(session, player)
	...
	-- Do not touch BeingChased: Slidemouth already owns that generic marker.
...
local function thirdPumpPosition(records)
	local state = stateFolder()
	local userId = state and state:GetAttribute("Level2_PumpActivatorUserId3")
	for _, record in ipairs(records) do
		if record.Player.UserId == userId then return record.Root.Position end
	end
	local captured = state and state:GetAttribute("Level2_PumpActivatorPosition3")
	return typeof(captured) == "Vector3" and captured or records[1].Root.Position
end

local function candidates(session, records)
	local result = {}
	local focus = thirdPumpPosition(records)
	for _, anchor in ipairs(session.Anchors) do
		...
		-- A hard exclusion for EVERY participant, not just the third activator.
...
local function navigationTuning(model)
	...
	return {AgentRadius = radius, PathAgentRadius = radius, AgentHeight = height,
		...
		RepathInterval = .65, FootClearance = .08, FloorProbeAbove = 18,
...
local function spawnAllowed(session)
	return roundReady(session) and workspace:GetAttribute("EntityPaused") ~= true
		and pumpCount() >= SPAWN_PUMPS and not session.Spawned
end
...
local function updateModel(session, deltaTime)
	...
	navigator:Step(dt, session.CurrentSpeed)
	local moved = distance(before, navigator:GetPosition())
	motion(session, running and "CHASE" or "APPROACH", moved > .01,
	...
	if now - session.ProgressAt >= 2 then
		if distance(session.ProgressPosition, navigator:GetPosition()) < 1 then
...
local function heartbeat(session, deltaTime)
	...
	if not roundReady(session) or workspace:GetAttribute("EntityPaused") == true then
		if session.Navigator then session.Navigator:Stop() end
		session.CurrentSpeed = 0
		setTarget(session, nil)
		motion(session, workspace:GetAttribute("EntityPaused") == true and "PAUSED" or "DORMANT",
			false, 0, "Idle")
		return
	end
