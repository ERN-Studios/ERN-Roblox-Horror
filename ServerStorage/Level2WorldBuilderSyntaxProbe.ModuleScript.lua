...
local function makeColumn(parent, position, height, radius, essential)
	...
	-- Tiled trumpet fillets.  A concave radius profile stays nearly vertical at
	-- the shaft, then opens quickly into the floor/ceiling like the molded
	-- Poolrooms capitals.  Thin overlapping cylinders keep the silhouette round
	-- without the ball-shaped bulge of the old flattened spheres.  The two axial
	-- cap faces are buried in neighbouring slices, so only the four visible
	-- barrel projections need tile textures.
	if not essential then
		local flareLength = math.clamp(radius * 1.15, 6.2, 8.4)
		local maximumScale = 2.2
		local ringCount = 8
		local ringHeight = flareLength / ringCount
		local barrelFaces = {
		...
		for _, endpoint in ipairs({
			...
			{Anchor = height, ShaftDirection = -1},
		}) do
			for ringIndex = 1, ringCount do
				local u = (ringIndex - 1) / (ringCount - 1)
				local scale = 1 + (maximumScale - 1) * u ^ 2.35
				local offset = ringHeight * .5 + (1 - u) * (flareLength - ringHeight)
				local centerY = endpoint.Anchor + endpoint.ShaftDirection * offset
...
local function visualOverlapFor(points, index, radius, minimum)
	...
	local bendOverlap = 2 * radius * math.tan(math.min(turnAngle, math.rad(60)) * .5) + .04
...
local function makeTubeFromPoints(parent, points, radius, color, name, openTop,
	visualOverlap, collisionPoints, safetyWallHeight)
	...
	for index = 1, #points - 1 do
		...
		if length > .05 then
			...
			configureSlideVisual(visual, template, name .. " Visual " .. suffix,
				base, radius, length, color,
				visualOverlapFor(points, index, radius, visualOverlap))
	...
	-- Closed tubes get one matching shell lip at each end. Open half-tubes end
	-- cleanly on their own; adding lips there created the stacked feathering at
	-- every water-park entrance.
	if not openTop and #points >= 2 then
		makeSlideMouth(visuals, name .. " End Rim A", points[1], points[2],
			radius * 1.02, color, 0, true)
...
function makeSlideMouth(parent, name, startPoint, nextPoint, radius, color, transparency, closed)
	local template = slideTemplate(not closed)
	if not template then
...
local function makeEntryTub(parent, mouthPoint, towardPoint, radius, deckTop, color, name)
	...
	-- A molded, slide-coloured cradle makes the open shell look structurally
	-- seated on its deck.  The low plinth touches the flume's underside while
	-- mirrored wedge gussets rise only OUTSIDE the rider channel, following the
	-- lime support silhouette from the reference without narrowing the slide.
	local cradleCenter = (rearPoint + mouthPoint) * .5
	...
	local base = part(parent, name .. " Entry Cradle Base",
		CFrame.lookAt(Vector3.new(cradleCenter.X, baseTop - baseThickness * .5, cradleCenter.Z),
			Vector3.new(mouthPoint.X, baseTop - baseThickness * .5, mouthPoint.Z), Vector3.yAxis),
		Vector3.new(radius * 2.15, baseThickness, entryLength + .35),
		color, Enum.Material.SmoothPlastic)
	base.Reflectance = Configuration.SlideMouthReflectance or .08
	base.CanCollide = false
	base.CanTouch = false
	base.CanQuery = false
	base:SetAttribute("Level2_SlideEntrySupport", true)

	local braceWidth = radius * .58
	local braceHeight = math.clamp(radius * .62, 1.7, 4.9)
	local braceLength = math.max(3.2, entryLength - .55)
	for sideSign = -1, 1, 2 do
		local outward = sideDirection * sideSign
		local outerEdge = radius + .16
		local braceCenter = cradleCenter
			+ outward * (outerEdge - braceWidth * .5)
			+ Vector3.new(0, baseTop + braceHeight * .5 - cradleCenter.Y, 0)
		local brace = Instance.new("WedgePart")
		brace.Name = name .. " Entry Cradle Brace " .. (sideSign < 0 and "Left" or "Right")
		brace.Anchored = true
		-- Reversing the length axis on the mirrored side keeps each basis
		-- right-handed while leaving the prism's visible shape unchanged.
		brace.CFrame = CFrame.fromMatrix(
			braceCenter, direction * sideSign, Vector3.yAxis, outward)
		brace.Size = Vector3.new(braceLength, braceHeight, braceWidth)
		brace.Color = color
		brace.Material = Enum.Material.SmoothPlastic
		brace.Reflectance = Configuration.SlideMouthReflectance or .08
		brace.TopSurface = Enum.SurfaceType.Smooth
		brace.BottomSurface = Enum.SurfaceType.Smooth
		brace.CanCollide = false
		brace.CanTouch = false
		brace.CanQuery = false
		brace:SetAttribute("Level2_SlideEntrySupport", true)
		brace.Parent = parent
	end
...
local function makeSlideHall(parent, hall, index, doors)
	...
	for slide = 1, Configuration.SlidesPerHall do
		...
		if math.abs(lane) <= laneLimit then
			...
			makeSlideTube(hallFolder, startPoint, p1, p2, p3, radius, color,
				...
				Configuration.SlideSegments * 3, true, .05)
	...
	if helixRadius >= 12 then
		...
		-- Extend into the east catwalk instead of merely sharing its mathematical
		-- edge.  The overlap removes the corner crack at every rendering scale.
		local bridgeOuterX = hall.MaxX - 14.5
	...
	-- Two studs of physical overlap close the former deck/catwalk L-joint hole.
	local catwalkZ1 = deckFront - 2
...
local function makeExitFlume(parent, layout, hall, deck)
	...
	makeWallCollar(shellX, Configuration.WallThickness, 2)
...
local function makePlayTowerKit(parent, hall, depth, origin, yaw, topY, color, name)
	...
	makeEntryTub(parent, at(Vector3.new(0, topY + 4.4, 6)),
		at(Vector3.new(0, topY + 4.4, 13.5)), 4.2, topY, color, name)
end
...
local function makeArrivalConcourse(parent, hall, roomDirection)
	...
	platform.CanCollide = true
	platform:SetAttribute("Level2_EntityGround", true)
