...
| [KF7FDmP1 upgrade cost](https://trello.com/c/KF7FDmP1) | `8ee95f4` | `ZyntraConfig.UpgradeCost(level) = level + 1`; owner approved the ladder. Published Player: two consecutive Stamina and two Battery buys at 12/13 Tokens each, 892→842 balance, next price 14 for both. Read-only Open Cloud DataStore read confirmed persisted 842/13/13. | Insufficient-balance refusal and new-server rejoin readback. |
...
- Trello [upgrade card](https://trello.com/c/KF7FDmP1) records the owner's approval of the `level + 1` Token cost and four published Player purchases (12/13 each for Stamina and Battery; [receipt](../artifacts/player-qa-v2083-20260924/README.md)). Read-only Open Cloud confirmed that the saved profile now holds 842 Tokens and level 13 in each upgrade. Insufficient-balance refusal and new-server rejoin still remain. The friend checklists are attached directly to the [physical mobile/tablet](https://trello.com/c/lUKD3R8G) and [controller](https://trello.com/c/uI8hg2At) cards. Fifteen Testing cards remain open after Level 3 moved there; none has all its documented live/physical/DataStore/purchase gates met.
