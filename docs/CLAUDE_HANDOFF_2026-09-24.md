...
## Later owner acceptance and sale status

On 24 September the owner accepted real-purchase, published-server and friend's physical-hardware checks without running them. Trello Done cards record this as an **acceptance waiver**, not a passed test. Do not treat the older "Still required before Done" column below as the current Trello status; it records the earlier handoff. Implementation or publication gaps and known defects were not waived.

All six Token Earner Game Passes were then enabled for sale at the previously approved prices. Two Roblox Open Cloud readbacks, including the [saved live sale receipt](../assets/monetization/booster-passes/live-sale-receipt-20260924.json), confirm `isForSale=true` for all six and prices 149/299/399 R$ plus 150/100/250 R$ upgrades. This is availability evidence, not evidence of a real purchase or DataStore rejoin. [Trello EtdsUM4e](https://trello.com/c/EtdsUM4e) is Done under the owner's waiver. The original creation receipt still truthfully records their earlier off-sale state.

The Level 2 radio-open cue was a possible source of processed speech. Commit `ebdbf09` preserves its SoundId but sets its volume to zero in Studio and repo. All 182 scripts compile, the focused Studio briefing layout check passed 213/213, and Source/editor match. The latest published place remains **v2083**, which does not contain this or the saved-only Level 3 maze/prompt and Signal Architect portrait changes. The owner is handling publication; do not mark those changes live from a Studio save.

Trello reconciliation after the waiver moved **12 of the 15 Testing cards** to Done, each with evidence and an explicit unperformed-test note. The remaining Testing cards are [developer suit](https://trello.com/c/SYUaXHKQ) (standing portrait saved in Studio, not published), [intro voice](https://trello.com/c/xKozYUIo) (saved-only code and radio-cue uncertainty), and [Level 3 maze](https://trello.com/c/3hEkFVUe) (saved-only layout and incomplete natural five-CD route). The rejected [Level 2 wall-depth card](https://trello.com/c/DDqjXkyU) remains To Do pending an owner-approved direction. The owner then asked Codex to stop testing and hand the remaining QA to Claude; the interrupted Level 3 Play session was stopped and Studio returned to Edit without changes.
...
| [EtdsUM4e Token Earner](https://trello.com/c/EtdsUM4e) | `91106da`, `d50033f`, `66ddc5a`, `cb762f2`, `c6cbbc7`, `8937264` | Studio (all passes granted) tier 5: clear +2 +8; at 2x +2 +2; 2x owner sees 3x 150 / 5x 250; wheel Token3 at 5x paid 15 with `PaidTokens`; the earlier COMING SOON state was from off-sale passes. The residual automatic-earnings fix is published in v2081 and keeps a durable bonus pending while ownership is unknown; offline `test_token_earner.py` 383. | Done by owner acceptance. Six passes are now on sale; [readback](../assets/monetization/booster-passes/live-sale-receipt-20260924.json). Real purchase/rejoin/bonus tests were waived, not performed. |
...
This was the earlier release gate: all six Token Earner passes were **off sale** and no purchases were made. The owner later waived the live purchase/rejoin checks and all six were enabled for sale; see the later acceptance update and readback above. No real purchase is claimed.
